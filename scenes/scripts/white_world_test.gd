...
# Per-track trim so all three sit at Waka Waka's loudness (Wavin Flag is ~15 dB
# hotter; Ole Ola already matches).
...
func _setup_music() -> void:
	...
	add_child(_music)
	# Single tracks must not loop on their own, so each one's end advances the
	# playlist; the playlist itself loops.
	for stream in FIGHT_MUSIC:
		if stream is AudioStreamMP3:
			stream.loop = false
	_music.finished.connect(_on_music_finished)
	# Voice line played over the win card after a minigame.
...
func _host_begin_pause(pid: int) -> void:
	# Never during a minigame (super) — _msub guards it, and so does the live
	# minigame instance, in case a request races in just as the super starts.
